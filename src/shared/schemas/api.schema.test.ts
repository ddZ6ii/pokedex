...
describe('ApiQueryParamsSchema', () => {
  ...
  describe('stats', () => {
    it('omits stat keys when stats is undefined', () => {
      const result = ApiQueryParamsSchema.parse({})
      const keys = Object.keys(result)
      expect(keys.some((k) => k.includes(':'))).toBe(false)
    })

    it('spreads stat query keys into output when stats are provided', () => {
      const result = ApiQueryParamsSchema.parse({ stats: { hp: [20, 80] } })
